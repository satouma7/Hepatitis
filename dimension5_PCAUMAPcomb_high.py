# 全サンプルを用いてPCA/UMAP計算した後、高精度スライドのみを可視化する
import numpy as np
import pandas as pd
...
Xpca_high = Xpca.transform(X_high)#PCAの結果を2次元配列化(highのみ)
Xumap_high = Xumap.transform(X_high)#UMAPの結果を2次元配列化(highのみ)
...
Cpca_high = Cpca_all[Call_high]
Cumap_high = Cumap_all[Call_high]
...
plt.savefig(out/"high_PCA_UMAP.png", dpi=200)
plt.show()