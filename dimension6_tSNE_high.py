...
#PCA100次元
Xpca100 = PCA(n_components=100, random_state=0).fit_transform(X_high)

# t-SNE
Xtsne = TSNE(
    n_components=2, perplexity=100, metric='cosine',init='pca', learning_rate='auto', random_state=0
).fit_transform(Xpca100)
...
scatter_by_slide(ax, Xtsne, id_high, lab_high, "tSNE (high-acc patches)",id2color)

# スライド番号を重心付近に表示
for sid in high_acc:
    sel = (id_high == sid)
    if np.any(sel):
        cx, cy = Xtsne[sel].mean(axis=0)
...
plt.savefig(out/"high_tSNE.png", dpi=200)
plt.show()